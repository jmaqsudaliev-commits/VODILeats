...
function renderCouriersTable() {
  ...
  if (currentCouriers.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:var(--text-muted); padding:24px;">Hozircha kuryerlar ro'yxatdan o'tmagan.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = currentCouriers.map((c, i) => `
    <tr>
      ...
      <td>${c.user ? (c.user.firstName + ' ' + (c.user.lastName || '')) : 'Kuryer'}</td>
